        ...
        return (
            <Container>
                <Breadcrumb style={{ margin: '16px 0' }}>
                    <Breadcrumb.Item><b>COMPANY</b></Breadcrumb.Item>
                </Breadcrumb>
                <div style={{ background: '#fff', padding: 24, minHeight: 280 }}>
                    <Table>
                        ...
                        {this.state.companies.map((company, index) =>
                            <tr key={index}>
                                <th>{index}</th>
                                <th><Link to={'/management/company/'.concat(company.id)} data-company-id={company.id} data-key={index} onClick={this.detailCompany.bind(this)}>{company.name}</Link></th>
